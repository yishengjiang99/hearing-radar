...
export function getReader(sharedRingBuffer: SharedRingBuffer) {
  let offset = sharedRingBuffer.rptr;
  const dv = new DataView(sharedRingBuffer.data);
  ...
  const fgets = (n: number) => (n > 1 ? fgetc() + fgets(n) : fgetc());
  const readVarLength = () => {
    let v = 0;
    let n = fgetc();
    v = n & 0x7f;
  ...
  return {
    ...
    fgets,
    readVarLength,
    readDone: () => offset < buffer.byteLength,
    update: (newBuffer, newOffset) => {
      buffer = newBuffer;
      offset = newOffset;
    },
  };
