...
import { spawnToBuffer } from "./ffmpeg-link";
...
export const midi_sample_mp3 = (instrument: string, midi: number): Buffer => {
	...
	const fd = openSync(
		`./mp3/Fatboy_${instrument.replace(" ", "_")}/${midi + 21}.mp3`,
		"r"
	);
...
export const midi_s16le_ac1_ar_9000 = async (
	instrument: string,
	midi: number
): Promise<Buffer> => {
	const cacheKey: string = instrument + midi;
	...
	const infile = `./mp3/Fatboy_${instrument.replace(" ", "_")}/${
		midi - 21
	}.mp3`;
	const ffm = spawn(
		"ffmpeg",
		`-i ${infile} -f s16le -ac 1 -ar 9000 -`.split(" ")
	);
...
export const combined_midi_s16le_ac1_ar_9000_proc = (
	notes: Note[],
	attributes: Attributes
): ChildProcess => {
	const cacheKey: string = notes
