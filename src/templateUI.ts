export function templateUI(): {
  ...
  const stdoutdiv: HTMLDivElement = document.querySelector(
    "#stdout"
  ) as HTMLDivElement;
  ...
  const cp = document.querySelector<HTMLDivElement>("div#cp") as HTMLDivElement;
  const stdin = document.querySelector<HTMLInputElement>(
    "input"
  ) as HTMLInputElement;
  ...
  function render() {
    ...
    stdoutdiv.innerHTML = stdoutBuffer.map((l) => `<div>${l}</div>`).join("");
  }
  const postRx1 = (str: string) => {
    if (rx1 === str) {
      return;
    }
    rx1 = str;
